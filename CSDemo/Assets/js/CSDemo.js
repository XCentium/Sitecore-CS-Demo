 ...
 */
	...
	function loadReviewFormData() {
		...
		if ($("#review-form").length) {
			...
			if (typeof (Cookies.get("shipping_form")) !== "undefined") {
				...
				if (formValues.digital_shipping_address && formValues.digital_shipping_address.length) {
					$("#shippingMethod").append("<li>Electronic Delivery: " + formValues.digital_shipping_address[0] + "<li>");
				}
	...
	function submitShippingFormData(thisObj) {
		...
		if ($("#shipping-form").valid()) {
			...
			if (formValues.radio_shipping_2_options && formValues.email_shipping_id) {
				var data = {
					physicalShippingId: formValues.radio_shipping_2_options[0],
					digitalShippingId: formValues.email_shipping_id[0],
					email: formValues.digital_shipping_address[0]
				};

				$.ajax({
					type: "POST",
					url: "/AJAX/cart.asmx/ApplyMixedShippingMethodToCart",
				...
				if (formValues.radio_shipping_2_options) {
					data.shippingMethodId = formValues.radio_shipping_2_options[0];
				} else {
					data.shippingMethodId = formValues.email_shipping_id[0];
					data.email = formValues.digital_shipping_address[0];
				}
