...
namespace CSDemo.Models.Checkout.Cart
{
	public class CartHelper
	{
		...
		/// <returns></returns>
		internal bool AddShippingMethodToCart(string shippingMethodId, string email = "")
		{
			var shippingData = shippingMethodId.Split('|');
			var cart = GetCustomerCart();

			var regularItems = cart.Lines.Where(c => string.IsNullOrEmpty(CSDemo.Models.Product.Product.GetProduct(c.Product.ProductId).Tags) || !CSDemo.Models.Product.Product.GetProduct(c.Product.ProductId).Tags.Contains("entitlement")).ToList();
			var digitalItems = cart.Lines.Where(c => !string.IsNullOrEmpty(CSDemo.Models.Product.Product.GetProduct(c.Product.ProductId).Tags) && CSDemo.Models.Product.Product.GetProduct(c.Product.ProductId).Tags.Contains("entitlement")).ToList();

			var preferenceType = digitalItems.Any() ? "3" : "1";
				...
				new ShippingMethodInputModelItem
					{
						...
						ShippingPreferenceType =preferenceType,
						 ElectronicDeliveryEmail = email,
						PartyID = "0"
					}
			};
			...
			var orderPreferenceType = InputModelExtension.GetShippingOptionType(preferenceType);
		...
		internal bool AddMixedShippingMethodToCart(string physicalShippingId, string digitalShippingId, string email)
		{
			var cart = GetCustomerCart();
			var shippingData = physicalShippingId.Split('|');
			var digitalShippingData = digitalShippingId.Split('|');
			...
			var shippingMethodList = new List<ShippingMethodInputModelItem>
			{
				...
				new ShippingMethodInputModelItem
				{
					...
					ElectronicDeliveryEmail = email,
					ElectronicDeliveryEmailContent = "Test",
			...
			//prepare shipping list - items to be shipped
			var internalShippingList = shippingMethodList.ToShippingInfoList();
			var orderPreferenceType = InputModelExtension.GetShippingOptionType("4");
			...
			foreach (var shipping in internalShippingList)
			{
				...
				{
					shipping.LineIDs = regularItems.Select(lineItem => lineItem.ExternalCartLineId).ToList().AsReadOnly();
				}
			}
