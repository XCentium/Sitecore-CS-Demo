...
namespace CSDemo.Models.Checkout.Cart
{
    ...
    public class GetShippingMethods : Sitecore.Commerce.Connect.CommerceServer.Orders.Pipelines.GetShippingMethods
    {
        ...
        public override void Process(Sitecore.Commerce.Pipelines.ServicePipelineArgs args)
        {
            ...
            string query = string.Format(CultureInfo.InvariantCulture, "fast:{0}//*[@{1} = '{2}']", shippingOptionsItem.Paths.FullPath, Constants.KnownFieldNames.ShippingOptionValue, request.ShippingOption.ShippingOptionType.Value);
            Item foundOption = shippingOptionsItem.Database.SelectSingleItem(query);
            if (foundOption != null)
            {
                string shippingMethodsIds = foundOption[Constants.KnownFieldNames.CommerceServerShippingMethods];
                ...
                if (request is CSDemo.Models.Checkout.Cart.GetShippingMethodsRequest && result is GetShippingMethodsResult)
                {
                    var obecRequest = (CSDemo.Models.Checkout.Cart.GetShippingMethodsRequest)request;
